...
use super::{
    place_new_window, ssd::HEADER_BAR_HEIGHT, FullscreenSurface, PointerMoveSurfaceGrab,
    PointerResizeSurfaceGrab, ResizeData, ResizeState, SurfaceData, TouchMoveSurfaceGrab, WindowElement,
};
...
impl<BackendData: Backend> XwmHandler for AnvilState<BackendData> {
    ...
    fn map_window_request(&mut self, _xwm: XwmId, window: X11Surface) {
        ...
        place_new_window(self.workspaces.space_mut(), self.human_pointer.current_location(), &window, true);
        let bbox = self.workspaces.space().element_bbox(&window).unwrap();
    ...
    fn mapped_override_redirect_window(&mut self, _xwm: XwmId, window: X11Surface) {
        ...
        let window = WindowElement(Window::new_x11_window(window));
        self.workspaces.space_mut().map_element(window, location, true);
    }
    ...
    fn unmapped_window(&mut self, _xwm: XwmId, window: X11Surface) {
        ...
        if let Some(elem) = maybe {
            self.workspaces.space_mut().unmap_elem(&elem)
        }
    ...
    fn configure_notify(
        ...
        self.workspaces.space_mut().map_element(elem, geometry.loc, false);
    ...
    fn unmaximize_request(&mut self, _xwm: XwmId, window: X11Surface) {
            ...
            .and_then(|data| data.restore())
        {
            ...
            self.workspaces.space_mut().map_element(elem, old_geo.loc, false);
        }
    }
    ...
    fn fullscreen_request(&mut self, _xwm: XwmId, window: X11Surface) {
        if let Some(elem) = self
            ...
            .find(|e| matches!(e.0.x11_surface(), Some(w) if w == &window))
        {
            let outputs_for_window = self.workspaces.space().outputs_for_element(elem);
            let output = outputs_for_window
                .first()
                // The window hasn't been mapped yet, use the primary output instead
                .or_else(|| self.workspaces.space().outputs().next())
                // Assumes that at least one output exists
                .expect("No outputs found");
            let geometry = self.workspaces.space().output_geometry(output).unwrap();
    ...
    fn unfullscreen_request(&mut self, _xwm: XwmId, window: X11Surface) {
            ...
            .find(|e| matches!(e.0.x11_surface(), Some(w) if w == &window))
        {
            ...
            if let Some(output) = self.workspaces.space().outputs().find(|o| {
                o.user_data()
                    .get::<FullscreenSurface>()
                    .and_then(|f| f.get())
                    .map(|w| &w == elem)
                ...
                window.configure(self.workspaces.space().element_bbox(elem)).unwrap();
    ...
    fn resize_request(&mut self, _xwm: XwmId, window: X11Surface, _button: u32, edges: X11ResizeEdge) {
        ...
        let start_data = self.human_pointer.grab_start_data().unwrap();
        ...
        let loc = self.workspaces.space().element_location(element).unwrap();
        ...
        let pointer = self.human_pointer.clone();
    ...
    fn allow_selection_access(&mut self, xwm: XwmId, _selection: SelectionTarget) -> bool {
        if let Some(keyboard) = self.human_seat.get_keyboard() {
    ...
    fn send_selection(&mut self, _xwm: XwmId, selection: SelectionTarget, mime_type: String, fd: OwnedFd) {
        match selection {
            SelectionTarget::Clipboard => {
                if let Err(err) = request_data_device_client_selection(&self.human_seat, mime_type, fd) {
                    error!(?err, "Failed to request current wayland clipboard for Xwayland",);
                }
            }
            SelectionTarget::Primary => {
                if let Err(err) = request_primary_client_selection(&self.human_seat, mime_type, fd) {
                    error!(
                        ?err,
    ...
    fn new_selection(&mut self, _xwm: XwmId, selection: SelectionTarget, mime_types: Vec<String>) {
        ...
        match selection {
            SelectionTarget::Clipboard => {
                set_data_device_selection(&self.display_handle, &self.human_seat, mime_types, ())
            }
            SelectionTarget::Primary => {
                set_primary_selection(&self.display_handle, &self.human_seat, mime_types, ())
            }
        }
    }
    ...
    fn cleared_selection(&mut self, _xwm: XwmId, selection: SelectionTarget) {
        match selection {
            SelectionTarget::Clipboard => {
                if current_data_device_selection_userdata(&self.human_seat).is_some() {
                    clear_data_device_selection(&self.display_handle, &self.human_seat)
                }
            }
            SelectionTarget::Primary => {
                if current_primary_selection_userdata(&self.human_seat).is_some() {
                    clear_primary_selection(&self.display_handle, &self.human_seat)
                }
            }
        }
    }
...
impl<BackendData: Backend> AnvilState<BackendData> {
    pub fn maximize_request_x11(&mut self, window: &X11Surface) {
        ...
        let old_geo = self.workspaces.space().element_bbox(&elem).unwrap();
        let outputs_for_window = self.workspaces.space().outputs_for_element(&elem);
        let output = outputs_for_window
            .first()
            // The window hasn't been mapped yet, use the primary output instead
            ...
            // Assumes that at least one output exists
            .expect("No outputs found");
        let geo = self.workspaces.space().output_geometry(output).unwrap();
        let geometry = {
        ...
        let is_ssd = elem.decoration_state().is_ssd;
        ...
        window.configure(Rectangle::new(geometry.loc, client_size)).unwrap();
    ...
    pub fn move_request_x11(&mut self, window: &X11Surface) {
        if let Some(touch) = self.human_seat.get_touch() {
            if let Some(start_data) = touch.grab_start_data() {
                ...
                if let Some(element) = element {
                    let mut initial_window_location = self.workspaces.space().element_location(element).unwrap();
        ...
        // luckily anvil only supports one seat anyway...
        let Some(start_data) = self.human_pointer.grab_start_data() else {
            return;
        };
        ...
        if window.is_maximized() {
            window.set_maximized(false).unwrap();
            let pos = self.human_pointer.current_location();
            initial_window_location = (pos.x as i32, pos.y as i32).into();
            if let Some(old_geo) = window
                .user_data()
                .get::<OldGeometry>()
